...
// v3.7.0 - 外部JSON読み込み + CSV取込/JSON書き出しツール
...
function renderJump(){
  ...
  bar.innerHTML = "";
  const items = (state.data[state.currentMaker]||[]).slice().sort((a,b)=>{
  ...
  groups.forEach(g=>{
    ...
    b.className="jump"; b.textContent = g===0?"0":String(g);
...
function renderList(){
  ...
  items.forEach(it=>{
    ...
    if(sec!==currentSection){
      ...
      anchor.style.height="1px"; anchor.style.marginTop="-6px";
...
function toCSV(items){
  const rows = ["number,hex"];
  items.forEach(it=> rows.push(`${it.number},${it.hex||""}`));
  return rows.join("\n");
}
